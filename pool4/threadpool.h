...
public:
    ...
    void setThreadSizeShrehold(int size);
...
private:
    ...
    int threadMaxSizeShreshold_;     // cached 模式下线程数量上限阈值
    std::atomic_int currentThreadNum_;  // 当前线程数量
    std::atomic_int spareThreadNum_;    // 空闲的线程数量
    ...
    std::condition_variable notExit_;

    PoolMode mode_; // 线程池的模式

    std::atomic_bool isPoolRunning_;    // 线程池当前的状态
};
